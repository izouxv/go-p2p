...
func TestAddrBookSaveLoad(t *testing.T) {
	...
	randAddrs := randNetAddressPairs(100)
...
func randNetAddressPairs(n int) []netAddressPair {
	randAddrs := make([]netAddressPair, n)
	for i := 0; i < n; i++ {
		randAddrs[i] = netAddressPair{addr: randIPv4Address(), src: randIPv4Address()}
	}
	return randAddrs
}
